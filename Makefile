...
# Regression tests for the alias-check script
group-alias-check-test:
	node scripts/test-check-tool-group-alias.mjs

# End-to-end alias workflow (extract if source exists + validate + regression tests)
group-alias-full:
	@if [ -n "$$OPENCLAW_REPO_DIR" ] || [ -n "$$CLAWDBOT_REPO_DIR" ] || [ -f ../clawdbot/src/agents/tool-policy.ts ]; then \
		node scripts/extract-tool-groups.mjs; \
	else \
		echo "Skipping extract-tool-groups.mjs (set OPENCLAW_REPO_DIR/CLAWDBOT_REPO_DIR or provide ../clawdbot/src/agents/tool-policy.ts)"; \
	fi
	node scripts/check-tool-group-alias.mjs
	node scripts/test-check-tool-group-alias.mjs
