------------------------------ MODULE PairingStoreHarnessV2 ------------------------------
EXTENDS Naturals, Sequences, FiniteSets

(******************************************************************************
* PairingStoreHarnessV2.tla
*
* Roadmap R1: Pairing store (DM pairing + allowlist store).
*
* Bounded, trace-based model:
*  - RequestPair creates a pending request and logs it in requestLog.
*  - Approve adds sender to allowFrom and logs approval in approveLog.
*  - Approve is allowed only if there exists a non-expired pending request.
*  - Tick advances time.
*
* Key invariants:
*  - Pending cap per channel.
*  - No approval for expired request (trace-based: approval implies a request
*    within TTL exists in requestLog).
******************************************************************************)
...
\* For every approval, there exists a request in the log whose age at approval time is < TTL.
